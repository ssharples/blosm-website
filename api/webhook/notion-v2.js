...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Extract lead data
    const leadData = extractLeadData(page_id, properties);
 ...
 */
function extractLeadData(page_id, properties) {
  return {
    pageId: page_id,
    businessName: properties['Business Name']?.title?.[0]?.plain_text || 'Unknown Business',
